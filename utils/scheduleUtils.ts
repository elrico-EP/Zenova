...
export const recalculateScheduleForMonth = (
    nurses: Nurse[],
    date: Date,
    agenda: Agenda,
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (isWorkday) {
            const isManuallyManaged = !!manuallyManagedDays[dateKey];
            ...
            if (isManuallyManaged) {
                // For manually managed days, just assign ADMIN to anyone left over and skip auto-balancing.
                ...
                // Original automatic assignment logic for days that are NOT manually managed
...
export const generateAndBalanceGaps = (
    ...
    for (let d = new Date(Date.UTC(year, 0, 1)); d.getUTCFullYear() === year; d.setUTCDate(d.getUTCDate() + 1)) {
        ...
        if (isWorkday && !isManuallyManaged) {
            nurses.forEach(nurse => {
